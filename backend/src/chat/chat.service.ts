...
export class ChatService {
  constructor(
    ...
    private readonly sessionModel: MongooseModel<ChatSessionDocument>,
    @InjectModel(ChatMessage.name)
    private readonly messageModel: MongooseModel<ChatMessageDocument>,
  ) {}
  ...
  private score(model: Model, msg: string): number {
    ...
    for (const check of checks) {
      ...
      if (hit) {
        if (check.typeMatch && model.types.includes(check.typeMatch))
          score += check.points;
        else if (
          check.tagMatch &&
          model.tags.some((t) => t.toLowerCase().includes(check.tagMatch!))
        )
          score += check.points;
        else if (!check.typeMatch && !check.tagMatch) {
          if (
            check.keywords.some((kw) =>
              [
                'cheap',
                'free',
                'budget',
  ...
  private buildText(msg: string, context?: ChatContextDto): string {
    if (context?.goal) {
      ...
      const budgetNote =
        budget && !budget.toLowerCase().includes('$500')
