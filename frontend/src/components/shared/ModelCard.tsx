...
export default function ModelCard({ model }: ModelCardProps) {
  ...
  return (
    <motion.div
      whileHover={{
        y: -3,
        boxShadow: "0 2px 12px rgba(0,0,0,0.09),0 8px 32px rgba(0,0,0,0.05)",
      }}
    ...
    >
      <div className="flex items-start justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div
              className="font-syne font-semibold text-[0.9rem] sm:text-base text-text1 truncate"
              style={{ letterSpacing: "-0.02em" }}
            >
              {model.name}
            </div>
            <div className="text-[0.68rem] sm:text-[0.75rem] text-text3 mt-0.5">
              {model.org}
            </div>
          </div>
        </div>
        {model.badge && (
          <span
            className={`text-[0.62rem] sm:text-[0.68rem] font-semibold px-1.5 sm:px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0 ml-1 ${badgeStyles[model.badgeClass] || "bg-gray-100 text-gray-600"}`}
          >
            {model.badge === "new"
              ? "New"
              : model.badge === "hot"
      ...
      <p className="text-[0.78rem] sm:text-[0.83rem] text-text2 leading-[1.55] mb-3 sm:mb-4 flex-1 line-clamp-3">
        {model.desc}
      </p>
      ...
      <div className="flex gap-1 sm:gap-1.5 flex-wrap mb-3 sm:mb-4">
        {model.tags.slice(0, 3).map((tag, i) => (
          <span
            key={tag}
            className={`text-[0.62rem] sm:text-[0.7rem] px-1.5 sm:px-2 py-0.5 rounded-full font-medium ${tagColorList[i % 4]}`}
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-black/[0.08] gap-1">
        ...
        <span className="text-[0.72rem] sm:text-[0.78rem] font-medium text-teal truncate mx-1">
          {model.price.split("/")[0]}
        </span>
        <button
          onClick={handleChat}
