  ...
  if (phase === "building") {
    return (
      <div className="px-4 sm:px-6 py-8 text-center border-t border-black/[0.08]">
        ...
        <div className="font-syne text-[0.95rem] font-bold text-text1 mb-1">
          Building your personalised query…
        </div>
        <p className="text-[0.78rem] text-text3">
          Taking you to Chat Hub right away
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="relative overflow-hidden border-t border-accent/15 bg-gradient-to-br from-accent-lt/90 via-white/40 to-transparent">
        <div className="px-4 sm:px-6 pt-4 pb-4">
          ...
          <h3
            className="font-syne text-[1rem] sm:text-[1.08rem] font-bold text-text1 text-center mb-2"
          ...
          <div className="flex flex-col gap-2 mb-4 max-w-md mx-auto">
            {[
              ...
              >
                ...
                <span className="text-[0.74rem] sm:text-[0.78rem] text-text2 leading-snug">
                  {row.text}
                </span>
              </div>
            ))}
          ...
          <div className="mb-3 max-w-md mx-auto">
            <div className="text-[0.7rem] text-text3 mb-1.5">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[0.85rem] font-semibold text-white border-none cursor-pointer font-instrument bg-gradient-to-r from-accent to-accent2 shadow-md hover:brightness-95 active:brightness-90 transition-all"
            >
  ...
  return (
    <div className="border-t border-black/[0.08]">
      <div className="px-4 sm:px-6 pt-3 pb-1">
        <p className="text-[0.67rem] font-bold uppercase tracking-wider text-accent mb-1">
          Step {stepIndex + 1}
        </p>
        <h4
          className="font-syne text-[0.92rem] sm:text-[0.98rem] font-bold text-text1 mb-1"
          style={{ letterSpacing: "-0.01em" }}
        >
          {q.q}
        </h4>
        {q.hint ? (
          <p className="text-[0.72rem] sm:text-[0.76rem] text-text3 italic mb-3">
            {q.hint}
          </p>
        ) : null}
          ...
          {q.opts.map((o) => (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-semibold text-[0.8rem] sm:text-[0.82rem] text-text1">
                  {o.l}
                </span>
                <span className="block text-[0.68rem] sm:text-[0.72rem] text-text3 mt-0.5 leading-snug">
                  {o.sub}
                </span>
              </span>
            </button>
      ...
      <div className="flex items-center gap-2 px-4 sm:px-6 py-2.5 border-t border-black/[0.08] bg-white/50">
        <div className="flex gap-1 items-center flex-wrap">
          {steps.map((step, i) => (
            <div
              key={step.k}
              className={`rounded-full transition-all ${i < stepIndex ? "w-3 h-1.5 bg-accent" : i === stepIndex ? "w-2 h-2 bg-accent scale-125" : "w-1.5 h-1.5 bg-black/15"}`}
            />
          ))}
        </div>
        <span className="flex-1 text-[0.7rem] text-text3 hidden sm:inline">
          {answers.task ? `“${answers.task}”` : ""}
        </span>
        <button
          type="button"
