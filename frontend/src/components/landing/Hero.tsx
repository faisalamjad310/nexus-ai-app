...
const getActionGrid = (t: any): ActionGridItem[] => [
  { icon: "💬", label: t("landing.actions.chat"), href: "/chat" },
  { icon: "🔍", label: t("landing.actions.compare"), href: "/marketplace" },
  { icon: "🤖", label: t("landing.actions.build"), href: "/agents" },
  { icon: "📊", label: t("landing.actions.analyse"), href: "/research" },
  { icon: "🖼", label: t("landing.actions.generate"), href: "/marketplace" },
  {
    icon: "✦",
    label: t("landing.actions.explore"),
    href: "/marketplace",
    dashed: true,
  },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    apiHeroOnboarding()
      ...
      .catch(() => {
        ...
        dispatch(
          showToast(
            "Could not load guided setup. You can still search as usual.",
          ),
        );
      });
    return () => {
  ...
  const pickFiles = (files: FileList | File[] | null) => {
    ...
    setAttachments((prev) => {
      const map = new Map(
        prev.map((a) => [`${a.name}-${a.size}-${a.type}`, a]),
      );
    ...
    setAttachmentFiles((prev) => {
      ...
      const newFiles = picked.filter(
        (f) => !existingKeys.has(`${f.name}-${f.size}-${f.type}`),
      );
  ...
  return (
    <section
      ...
      style={{
        background:
          "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(115,115,115,0.08) 0%, transparent 70%)",
      }}
    >
      <div
      ...
      <motion.h1
        ...
        className="font-syne font-bold text-text1 max-w-[90vw] sm:max-w-[min(92vw,640px)] mb-5 sm:mb-6 relative z-10 px-2 flex flex-col items-center gap-0.5 sm:gap-1"
        style={{
      ...
      >
        <span className="block">{t("landing.hero_headline_l1")}</span>
        <span className="block text-accent">
          {t("landing.hero_headline_highlight")}
        </span>
        <span className="block">{t("landing.hero_headline_l2")}</span>
        <span className="block">{t("landing.hero_headline_l3")}</span>
      </motion.h1>
      ...
      <motion.p
        ...
        className="text-[0.88rem] sm:text-[1.02rem] text-text2/95 max-w-[90vw] sm:max-w-[560px] leading-relaxed mb-8 sm:mb-12 relative z-10 px-2 font-instrument"
      >
      ...
      >
        <div
          className={`bg-white border-[1.5px] rounded-[28px] shadow-md transition-all ${searchCardActive ? "border-accent shadow-[0_0_0_4px_rgba(115,115,115,0.14)]" : "border-black/[0.14]"}`}
        >
          ...
          <div className="flex items-center min-h-[52px] sm:min-h-[58px] relative">
            <FiSearch
              size={15}
              className="ml-4 sm:ml-5 text-text3 flex-shrink-0"
            />
            <input
              ref={inputRef}
              value={query}
            ...
            <div className="flex items-center gap-1.5 px-1 flex-shrink-0">
              ...
              <ToolbarTooltipButton
                ...
                className={`w-[38px] h-[38px] rounded-[12px] flex items-center justify-center transition-all cursor-pointer border shadow-sm ${
                  isRecording
                    ? "text-red-600 bg-red-50 border-red-200/90 animate-pulse"
                    : "bg-[#EDEAF3] border-[#C4B8D4] text-[#5B4B7A] hover:bg-[#E8E4F0] hover:border-[#B5A8C9]"
                }`}
                aria-label={isRecording ? "Stop recording" : "Record voice"}
              >
                <FiMic size={17} strokeWidth={2.25} />
              </ToolbarTooltipButton>
              <ToolbarTooltipButton
                tooltip={t("common.tooltips.attach_file")}
                onClick={() => fileInputRef.current?.click()}
                className="w-[38px] h-[38px] rounded-[12px] flex items-center justify-center cursor-pointer border shadow-sm bg-[#FFF4EB] border-[#E8C4A8] text-[#B45309] hover:bg-[#FFEFE3] hover:border-[#DEB896] transition-all"
                aria-label="Attach files"
              >
                <FiPaperclip size={17} strokeWidth={2.25} />
              </ToolbarTooltipButton>
              <ToolbarTooltipButton
                tooltip={t("common.tooltips.attach_image")}
                onClick={() => imageInputRef.current?.click()}
                className="hidden sm:inline-flex w-[38px] h-[38px] rounded-[12px] items-center justify-center cursor-pointer border shadow-sm bg-[#EAF4FB] border-[#B8D4EA] text-[#2563AB] hover:bg-[#E3F0FA] hover:border-[#9FC2E0] transition-all"
                aria-label="Attach images"
              >
                <FiImage size={17} strokeWidth={2.25} />
              </ToolbarTooltipButton>
              <ToolbarTooltipButton
                ...
                className={`w-[38px] h-[38px] rounded-[12px] flex items-center justify-center transition-all cursor-pointer border shadow-sm ${
                  voiceTypingActive
                    ...
                    : "bg-[#E8F5F0] border-[#9DC4B4] text-[#0F5E49] hover:bg-[#E0F2EB] hover:border-[#8BB8A8]"
                }`}
                aria-label={
                  voiceTypingActive ? "Stop voice typing" : "Voice typing"
                }
              >
            ...
            <span className="relative inline-flex group mr-1.5">
              <motion.button
                ...
                className="bg-accent text-white flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 rounded-[22px] text-[0.78rem] sm:text-[0.875rem] font-medium hover:bg-accent2 transition-colors border-none cursor-pointer font-instrument whitespace-nowrap"
                aria-label={t("common.tooltips.search")}
              >
                <FiSearch size={14} strokeWidth={2.25} aria-hidden />
      ...
      >
        {ACTION_GRID.map((a, i) => (
          <motion.button
            type="button"
            key={`action-${i}-${a.dashed ? "d" : "s"}`}
            whileHover={{
              y: -2,
              boxShadow: "0 6px 18px rgba(115,115,115,0.18)",
            }}
            onClick={() => router.push(a.href)}
            className={`flex min-w-0 flex-col items-center gap-1 sm:gap-1.5 bg-white border-[1.5px] border-black/[0.14] rounded-lg px-2 sm:px-3 py-3 sm:py-3.5 cursor-pointer transition-all shadow-card hover:bg-accent-lt hover:border-accent font-instrument overflow-hidden ${a.dashed ? "border-dashed bg-bg" : ""}`}
            style={{ borderRadius: 16 }}
          >
            <span className="text-xl sm:text-[1.4rem] leading-none shrink-0">
              {a.icon}
            </span>
            <span className="w-full min-w-0 text-[0.62rem] sm:text-[0.72rem] font-semibold text-text1 text-center leading-snug break-words hyphens-auto px-0.5">
              {a.label}
            </span>
          </motion.button>
        ))}
      ...
      >
        {[
          ...
          <div key={s.label} className="text-center">
            <strong className="block font-syne text-[1.5rem] sm:text-[2rem] font-bold text-text1">
              {s.value}
            </strong>
            <span className="text-[0.7rem] sm:text-[0.78rem] text-text3">
              {s.label}
            </span>
          </div>
        ))}
