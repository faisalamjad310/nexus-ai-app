...
export default function FeaturedModels() {
  ...
  const { items, labs, status: catalogStatus } = useSelector(
    (s: RootState) => s.models,
  );
  const featured = items.slice(0, 3);
  const catalogLoading =
    catalogStatus === "loading" ||
    (catalogStatus === "idle" && items.length === 0 && labs.length === 0);
  const catalogError = catalogStatus === "error";
  return (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {catalogLoading &&
            [0, 1, 2].map((i) => (
              <div
                key={`f-sk-${i}`}
                className="rounded-xl border border-black/[0.08] bg-white p-4 shadow-card min-h-[200px]"
              >
                <div className="h-36 rounded-lg bg-bg3/90 animate-pulse mb-3" />
                <div className="h-4 w-2/3 rounded bg-bg3/90 animate-pulse mb-2" />
                <div className="h-3 w-full rounded bg-bg3/70 animate-pulse" />
              </div>
            ))}
          {catalogError && !catalogLoading && (
            <p className="col-span-full text-sm text-text3">
              {t("landing.catalog_error")}
            </p>
          )}
          {!catalogLoading &&
            !catalogError &&
      ...
      <section className="py-10 sm:py-16 px-4 sm:px-8 md:px-10 bg-white">
        <div className="flex items-end justify-between mb-5 sm:mb-8">
          <div>
            ...
            <p className="text-[0.82rem] sm:text-[0.85rem] text-text2 mt-1">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">
          {catalogLoading &&
            Array.from({ length: 12 }).map((_, i) => (
              ...
              >
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-[8px] sm:rounded-[10px] bg-bg3/90 animate-pulse flex-shrink-0" />
                <div className="min-w-0 flex-1 space-y-2">
                  <div className="h-3.5 rounded bg-bg3/90 animate-pulse w-3/4" />
                  <div className="h-2.5 rounded bg-bg3/70 animate-pulse w-1/2" />
                </div>
              </div>
            ))}
          {!catalogLoading &&
            ...
            labs.map((lab, i) => (
              <motion.button
                ...
                whileHover={{
                  y: -2,
                  boxShadow: "0 6px 18px rgba(115,115,115,0.16)",
                }}
                onClick={() => dispatch(openApp("marketplace"))}
              ...
              >
                <div
                  className="w-8 h-8 sm:w-10 sm:h-10 rounded-[8px] sm:rounded-[10px] flex items-center justify-center text-base sm:text-lg flex-shrink-0"
                  style={{ background: lab.color }}
                >
                  {lab.icon}
                </div>
                <div className="min-w-0">
                  <div className="font-semibold text-[0.78rem] sm:text-sm text-text1 truncate">
                    {lab.name}
                  </div>
                  <div className="text-[0.65rem] sm:text-[0.7rem] text-text3">
                    {lab.count} models
                  </div>
                </div>
              </motion.button>
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(135deg, #F3F4F6 0%, rgba(115,115,115,0.06) 100%)",
        }}
      >
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <h2
            className="font-syne text-[1.6rem] sm:text-[2.2rem] font-bold text-text1 mb-3 sm:mb-4"
            style={{ letterSpacing: "-0.03em" }}
          >
