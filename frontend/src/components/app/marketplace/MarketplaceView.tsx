...
function ModelCardSkeleton() {
  return (
    <div
      className="bg-white border border-black/[0.08] p-4 sm:p-6 shadow-card h-[160px] sm:h-[180px] flex flex-col"
...
export default function MarketplaceView() {
  ...
  const {
    ...
    labs,
    status,
  } = useSelector((s: RootState) => s.models);
  const [showFilters, setShowFilters] = useState(false);
  const TYPE_FILTERS: TypeFilter[] = [
    {
      id: "all",
      label: t("marketplace.filters.all"),
      translationKey: "marketplace.filters.all",
    },
    {
      id: "language",
      label: t("marketplace.filters.language"),
      translationKey: "marketplace.filters.language",
    },
    {
      id: "vision",
      label: t("marketplace.filters.vision"),
      translationKey: "marketplace.filters.vision",
    },
    {
      id: "code",
      label: t("marketplace.filters.code"),
      translationKey: "marketplace.filters.code",
    },
    {
      id: "image",
      label: t("marketplace.filters.image_gen"),
      translationKey: "marketplace.filters.image_gen",
    },
    {
      id: "audio",
      label: t("marketplace.filters.audio"),
      translationKey: "marketplace.filters.audio",
    },
    {
      id: "open",
      label: t("marketplace.filters.opensource"),
      translationKey: "marketplace.filters.opensource",
    },
  ];
  const catalog = items;
  const filtered = useMemo(
    () =>
      catalog.filter((m) => {
        ...
        const matchType =
          activeFilter === "all" || m.types.includes(activeFilter);
        const matchLab = activeLab === "all" || m.lab === activeLab;
        const matchPrice = m.price_start <= priceRange;
        const matchRating =
          minRating === "any" ||
          (minRating === "4+" && m.rating >= 4) ||
          (minRating === "4.5+" && m.rating >= 4.5);
        return (
          matchSearch && matchType && matchLab && matchPrice && matchRating
        );
      }),
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="bg-white border-b border-black/[0.08] px-3 sm:px-6 py-3 sm:py-5 flex flex-col gap-3 flex-shrink-0">
        <div className="flex items-center gap-3 flex-wrap">
          <span
            className="font-syne text-[1rem] sm:text-[1.1rem] font-bold text-text1 whitespace-nowrap"
          ...
          <div className="flex-1 min-w-[160px] flex items-center bg-bg border border-black/[0.14] rounded-full focus-within:border-accent focus-within:bg-white transition-all overflow-hidden">
            <div className="pl-3 flex items-center text-text3 flex-shrink-0">
              <FiSearch size={13} />
            </div>
            <input
              value={searchQuery}
            ...
            />
            <button
              onClick={() => dispatch(showToast("Voice search — coming soon"))}
            ...
            <button
              onClick={() => dispatch(showToast("File upload — coming soon"))}
        ...
        {showFilters && (
          ...
          >
            <div className="p-3 space-y-3">
              <div>
                <div className="text-[0.68rem] font-semibold text-text3 uppercase tracking-wider mb-1.5">
                  Type
                </div>
              ...
              <div>
                <div className="text-[0.68rem] font-semibold text-text3 uppercase tracking-wider mb-1">
                  Price /1M tokens
                </div>
                <input
                  type="range"
                ...
                />
                <div className="text-[0.75rem] text-text2">
                  Up to ${priceRange}
                </div>
              </div>
      ...
      <div className="flex items-center gap-2 px-3 sm:px-5 py-2.5 sm:py-3 bg-white border-b border-black/[0.08] overflow-x-auto flex-shrink-0 scrollbar-none">
        ...
        <button
          onClick={() => dispatch(clearLabFilter())}
      ...
      <AnimatePresence>
        {activeLab !== "all" && (
          ...
          >
            ...
            <span>
              Showing <strong>{activeLab}</strong> models
            </span>
            <button
              onClick={() => dispatch(clearLabFilter())}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="hidden sm:flex flex-col w-[200px] lg:w-[220px] flex-shrink-0 p-3 sm:p-4 border-r border-black/[0.08] bg-white overflow-y-auto">
          ...
          >
            <div className="text-[0.75rem] font-semibold text-accent group-hover:text-white mb-0.5">
              ✦ Need help choosing?
            </div>
            <div className="text-[0.68rem] text-text2 group-hover:text-white/80 leading-snug">
              Chat with our AI guide for personalised picks.
            </div>
          </div>

          <div className="mb-4">
            <div className="text-[0.68rem] font-semibold text-text3 uppercase tracking-[0.06em] mb-1.5">
              Max Price /1M
            </div>
            <input
              type="range"
              ...
              className="w-full accent-accent"
            />
            <div className="text-[0.75rem] text-text2 mt-0.5">
              Up to ${priceRange}
            </div>
          </div>
            ...
            <div className="flex flex-col gap-1">
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    const m = items[0];
                    if (m) dispatch(openModal({ model: m, tab: g.tab }));
                  }}
