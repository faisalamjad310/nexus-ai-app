...
export default function AppNav() {
  ...
  const pathTab = TABS.find((tab) => {
    ...
    if (tab.id === "chat" && (pathname === "/chat" || pathname === "/chathub"))
      return true;
        ...
        {isAuthenticated && user ? (
          <>
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-bg border border-black/[0.1]">
              ...
              <span className="text-[0.8rem] text-text1 font-instrument max-w-[120px] truncate">
                {user.name}
              </span>
            </div>
            <button
          ...
          >
            ...
            <div className="px-3 pb-3 flex gap-2">
              {isAuthenticated && user ? (
                ...
                >
                  <FiLogOut size={13} />{" "}
                  {t("common.sign_in").includes("Sign")
                    ? "Sign out"
                    : "لاگ آؤٹ"}
                </button>
              ) : (
                <>
                  ...
                  <button
                    onClick={() => {
                      const m =
                        models.find((m) => m.id === "gpt5") || models[0];
                      if (m) dispatch(openModal({ model: m }));
                      setMobileOpen(false);
                    }}
