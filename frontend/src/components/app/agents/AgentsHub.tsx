...
import AgentBuilderWizard from "@/components/app/agents/AgentBuilderWizard";
import AgentConversationPanel from "@/components/app/agents/AgentConversationPanel";
...
export default function AgentsHub() {
  ...
  const {
    ...
    agents: userAgents,
    userAgentsStatus,
  } = useSelector((s: RootState) => s.agent);
  ...
  const [suggestionPermutation, setSuggestionPermutation] = useState<
    number[] | null
  >(null);
  ...
  const tabLabel = useCallback(
    (id: AgentExploreTabId) => {
      ...
      const tr = t(key);
      if (tr === key)
        return explore?.tabs.find((x) => x.id === id)?.label ?? id;
      return tr;
    },
    [explore, t],
  );
  ...
  const suggestionsOrdered = useMemo(() => {
    if (
      !suggestionPermutation ||
      suggestionPermutation.length !== baseSuggestions.length
    ) {
  ...
  const tplForGrid = templates.filter(
    (tmpl) => tmpl.title !== "Start from Scratch",
  );
  const exploreSubmit = () => {
    sendToChat(exploreSearch || t("agents.search_placeholder"));
    setExploreSearch("");
  };
    ...
    <div className="flex flex-1 h-full min-h-0 overflow-hidden bg-bg flex-col lg:flex-row">
      <aside className="hidden lg:flex w-full lg:w-[min(280px,32vw)] xl:w-[280px] shrink-0 flex-col gap-4 border-b lg:border-b-0 lg:border-r border-black/[0.08] bg-white px-4 sm:px-5 py-5 lg:py-6 overflow-y-auto max-h-[min(52vh,420px)] lg:max-h-none">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 bg-accent rounded-lg flex items-center justify-center text-xl text-white shrink-0">
            🤖
          </div>
          <div>
            ...
            <p className="text-[0.72rem] text-text2 leading-snug mt-0.5">
              {t("agents.sidebar_blurb")}
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-accent/25 bg-accent-lt p-3.5 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[0.8rem] font-semibold text-text1">
              {t("agents.banner_title")}
            </span>
          </div>
          <p className="text-[0.74rem] text-text2 leading-relaxed pl-7">
            {t("agents.banner_description")}
          </p>
          <button
            type="button"
        ...
        {libraryOpen ? (
          <div className="flex flex-1 min-h-0 overflow-hidden flex-col md:flex-row">
            <div className="w-full md:w-[260px] md:shrink-0 flex flex-col min-h-0 max-h-[min(42vh,340px)] md:max-h-none border-b md:border-b-0 md:border-r border-black/[0.08] bg-white">
              <div className="flex items-center justify-between px-3 sm:px-4 pt-4 sm:pt-5 pb-2 sm:pb-3 shrink-0">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-accent rounded-lg flex items-center justify-center text-sm text-white">
                    🤖
                  </div>
                  <span className="font-syne font-bold text-text1 text-[0.95rem] tracking-tight">
                    {t("agents.my_agents")}
                  </span>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-2 sm:px-2.5 pb-2 min-h-0 overscroll-contain">
                {userAgentsStatus === "loading" ? (
                  ...
                  <div className="text-center px-3 py-8 text-text3">
                    ...
                    <div className="text-[0.78rem] font-medium text-text2">
                      {t("agents.no_agents_title")}
                    </div>
                    <div className="text-[0.72rem] mt-1 leading-snug">
                      {t("agents.no_agents_hint")}
                    </div>
                  </div>
                ) : (
                    ...
                    <div
                      ...
                      className={`w-full rounded-lg border px-2.5 py-2 mb-1.5 flex items-center gap-2 transition-all ${
                        selectedAgentId === agent._id
                          ? "border-accent bg-accent-lt shadow-[0_1px_6px_rgba(115,115,115,0.14)]"
                    ...
                    >
                      ...
                      >
                        <span className="text-lg shrink-0">
                          {models.find((m) => m.id === agent.modelId)?.icon ??
                            "🤖"}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="text-[0.8rem] font-semibold text-text1 truncate">
                            {agent.name}
                          </div>
                        ...
                        <span
                          className={`w-2 h-2 rounded-full shrink-0 ${agent.status === "active" ? "bg-green" : agent.status === "paused" ? "bg-text3" : "bg-amber"}`}
                        />
                      </button>
                      <button
                        type="button"
            ...
            <div className="flex-1 flex flex-col min-w-0 min-h-0">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-3 sm:px-5 md:px-6 py-3 sm:py-4 border-b border-black/[0.08] bg-white shrink-0">
                <div className="min-w-0">
                  <div className="font-syne font-bold text-text1 text-[0.95rem] sm:text-[1.02rem] tracking-tight">
                    {t("agents.agent_library")}
                  </div>
                  <div className="text-[0.7rem] sm:text-[0.72rem] text-text2 mt-0.5 leading-snug">
                    {t("agents.library_sub")}
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-5 min-h-0 overscroll-contain">
                {catalogStatus === "loading" ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2.5 sm:gap-3">
                    {tplForGrid.map((tmpl) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1.5 min-w-0">
                          ...
                          <span className="font-syne font-semibold text-text1 text-[0.78rem] sm:text-[0.82rem] truncate">
                            {tmpl.title}
                          </span>
                        </div>
                        <p className="text-[0.7rem] text-text2 leading-relaxed line-clamp-2 mb-2">
                          {tmpl.desc}
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {tmpl.tags.slice(0, 3).map((tag) => (
                            <span
                              key={tag}
                              className={`text-[0.6rem] px-2 py-0.5 rounded-full font-medium ${getTagColor(tag)}`}
                            >
                              {tag}
                            </span>
                          ))}
                    ...
                    <motion.button
                      type="button"
                      whileHover={{ background: "rgba(115,115,115,0.1)" }}
                      onClick={() => {
                        handleScratchCard();
                        closeLibrary();
                      }}
          ...
          <>
            <div className="lg:hidden flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 border-b border-black/[0.08] bg-white shrink-0">
              <span className="font-syne font-bold text-text1 text-[0.9rem] sm:text-[0.95rem] min-w-0 truncate">
                {t("common.agents")}
              </span>
              <button
                type="button"
              ...
              >
                <FiPlus size={14} />{" "}
                <span className="max-[340px]:hidden sm:inline">
                  {t("agents.button_new_agent")}
                </span>
              </button>
            </div>
                ...
                {appsDrawerOpen ? (
                  ...
                  >
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-black/[0.08] shrink-0">
                      ...
                      <div>
                        <div className="font-syne font-bold text-text1">
                          {t("agents.use_case_library")}
                        </div>
                        <div className="text-[0.72rem] text-text2">
                          {t("agents.apps_count", {
                            count: useCaseApps.length,
                          })}{" "}
                          · {tabLabel(activeTab)}
                        </div>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3 content-start bg-bg overscroll-contain">
                      {useCaseApps.map((app, i) => (
                        <motion.button
                          ...
                          onClick={() =>
                            sendToChat(
                              `Help me build: ${app.name}. ${app.desc}`,
                            )
                          }
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="font-syne font-semibold text-[0.82rem] text-text1 leading-tight">
                              {app.name}
                            </span>
                          </div>
                          <div className="text-[0.65rem] font-medium text-accent mb-1">
                            {app.type}
                          </div>
                          <p className="text-[0.72rem] text-text2 leading-snug line-clamp-3">
                            {app.desc}
                          </p>
                        </motion.button>
                      ))}
                  ...
                  >
                    <div className="text-center px-4 sm:px-6 pt-5 sm:pt-8 pb-3 sm:pb-4 bg-[radial-gradient(ellipse_80%_60%_at_50%_-10%,rgba(115,115,115,0.08)_0%,transparent_70%)] shrink-0">
                      <h1 className="font-syne text-[clamp(1.35rem,4.5vw,2rem)] font-bold text-text1 tracking-tight leading-tight px-1">
                        ...
                        <span className="text-accent">
                          {t("agents.hero_accent")}
                        </span>
                      </h1>
                      <p className="text-[0.8rem] sm:text-[0.85rem] text-text2 mt-2 max-w-md mx-auto leading-relaxed px-1">
                        {t("agents.hero_sub")}
                      </p>
                    </div>
                          ...
                          {userAgents.map((a) => (
                            ...
                            >
                              <span className="text-base shrink-0">
                                {models.find((m) => m.id === a.modelId)?.icon ??
                                  "🤖"}
                              </span>
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:gap-2 sm:items-end max-w-3xl mx-auto w-full">
                        ...
                        >
                          ...
                          <span className="sm:hidden">
                            {t("agents.search_send")}
                          </span>
                        </motion.button>
                      </div>
                      {exploreLoadError && (
                        <p className="text-center text-[0.7rem] text-amber mt-2">
                          Could not load explore content; templates still work.
                        </p>
                      )}
                    </div>
                      ...
                      {tabIds.map((id) => (
                        <button
                          ...
                          className={`flex items-center gap-1 sm:gap-1.5 shrink-0 snap-start rounded-full px-2.5 sm:px-3 py-1.5 text-[0.68rem] sm:text-[0.74rem] font-semibold border transition-all cursor-pointer font-instrument whitespace-nowrap ${
                            activeTab === id
                              ? "bg-accent text-white border-accent shadow-[0_2px_10px_rgba(115,115,115,0.22)]"
                    ...
                    <div className="px-3 sm:px-6 pb-2 flex-1 min-h-0 bg-white">
                      <div className="max-w-3xl mx-auto space-y-2 py-2">
                        {filteredSuggestions.map((s) => (
                          ...
                          >
                            <span className="text-base sm:text-lg shrink-0 leading-none mt-0.5">
                              {s.icon}
                            </span>
                            <span className="text-[0.78rem] sm:text-[0.82rem] text-text2 leading-snug min-w-0 flex-1 break-words">
                              {s.text}
                            </span>
                            <FiChevronRight
                              className="shrink-0 text-text3 mt-0.5 sm:mt-1"
                              size={14}
                            />
                          </button>
                        ))}
            ...
            <div className="border-t border-black/[0.08] bg-bg shrink-0 max-h-[min(32dvh,220px)] sm:max-h-[220px] flex flex-col min-h-0">
              <div className="flex items-center gap-2 px-3 sm:px-4 pt-2 sm:pt-3 pb-1 shrink-0">
                <span className="font-syne text-[0.68rem] sm:text-[0.72rem] font-bold text-text3 uppercase tracking-[0.06em]">
                  {t("agents.templates_label")}
                </span>
                <span className="text-[0.62rem] sm:text-[0.65rem] bg-bg2 text-text3 px-1.5 py-0.5 rounded">
                  {tplForGrid.length}
                </span>
              </div>
              <div className="overflow-x-auto px-2 sm:px-3 pb-2 sm:pb-3 flex gap-2 sm:gap-2.5 scrollbar-none snap-x snap-mandatory touch-pan-x">
                {catalogStatus === "loading"
                  ? Array.from({ length: 5 }).map((_, i) => (
                      <Skeleton
                        ...
                        height={100}
                        borderRadius={12}
                        className="shrink-0 snap-start w-[min(200px,78vw)]"
                      />
                    ))
                  : tplForGrid.slice(0, 8).map((tmpl) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="font-syne font-semibold text-[0.75rem] text-text1 truncate">
                            {tmpl.title}
                          </span>
                        </div>
                        <p className="text-[0.65rem] text-text2 line-clamp-2 leading-snug">
                          {tmpl.desc}
                        </p>
                      </motion.button>
                    ))}
                <motion.button
                  type="button"
                  whileHover={{ background: "rgba(115,115,115,0.12)" }}
                ...
                >
                  ...
                  <span className="text-[0.72rem] font-semibold text-accent text-center">
                    Scratch
                  </span>
                </motion.button>
              </div>
