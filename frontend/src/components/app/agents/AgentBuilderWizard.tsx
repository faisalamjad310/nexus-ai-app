...
const TOOLS = [
  ...
  },
  {
    ...
    desc: "Execute and debug code automatically",
  },
  {
    id: "file_access",
    icon: "📂",
    label: "File Access",
    desc: "Read and write files & documents",
  },
  {
    id: "api_calls",
    icon: "🔗",
    label: "API Calls",
    desc: "Connect to external REST APIs",
  },
  {
    id: "memory",
    icon: "🧠",
  ...
  },
  {
    ...
    icon: "⏰",
    label: "Scheduler",
    desc: "Run tasks on a timed schedule",
  },
];
export default function AgentBuilderWizard() {
  const dispatch = useDispatch();
  const { builderOpen, builderStep, draft, editingId, runResult, runLoading } =
  ...
  const canNext = () => {
    if (builderStep === "configure")
      return draft.name.trim().length >= 2 && draft.modelId;
    return true;
  };
  const handleNext = () => {
    ...
    <AnimatePresence>
      {builderOpen && (
        <motion.div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && dispatch(closeBuilder())
          }
        >
          <motion.div
          ...
          >
            ...
            <div className="px-5 sm:px-6 py-4 border-b border-black/[0.08] flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2
                    className="font-syne font-bold text-[1rem] text-text1"
                    style={{ letterSpacing: "-0.02em" }}
                  >
                    {editingId ? "Edit Agent" : "New Agent"}
                  </h2>
                  <p className="text-[0.7rem] text-text3">
            ...
            <div className="px-5 sm:px-6 py-3 border-b border-black/[0.08] flex items-center gap-1 flex-shrink-0 overflow-x-auto">
              {STEPS.map((step, i) => (
                ...
                >
                  <div
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[0.72rem] font-medium transition-all ${
                      i === stepIndex
                  ...
                  {i < STEPS.length - 1 && (
                    <div className="w-4 h-px bg-black/[0.1]" />
                  )}
                </div>
              ))}
            </div>
...
function TemplateStep() {
  ...
  return (
    <div>
      <p className="text-[0.82rem] text-text2 mb-4">
        Choose a template to pre-fill your agent, or start from scratch.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        {status === "loading"
          ? Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="flex items-start gap-3 p-3.5 border border-black/[0.1] rounded-xl bg-bg h-[70px]"
              >
          ...
          : templates.map((tmpl) => (
              <motion.button
                key={tmpl.title}
                whileHover={{ borderColor: "#737373", background: "#F3F4F6" }}
                whileTap={{ scale: 0.98 }}
                onClick={() =>
                  dispatch(
                    applyTemplate({
                      name:
                        tmpl.title === "Start from Scratch" ? "" : tmpl.title,
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="text-[0.82rem] font-semibold text-text1 mb-0.5">
                    {tmpl.title}
                  </div>
                  <div className="text-[0.72rem] text-text2 leading-snug">
                    {tmpl.desc}
                  </div>
                </div>
              </motion.button>
...
function ConfigureStep({ catalog }: { catalog: Model[] }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-[0.78rem] font-medium text-text2 mb-1.5">
          Agent Name *
        </label>
        <input
          value={draft.name}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {catalog
            .filter(
              (m) => m.types.includes("language") || m.types.includes("code"),
            )
            .slice(0, 6)
            .map((m) => (
              <motion.button
      ...
      <div>
        <label className="block text-[0.78rem] font-medium text-text2 mb-2">
          Tools & Capabilities
        </label>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {TOOLS.map((tool) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <div
                    className={`text-[0.72rem] font-semibold ${active ? "text-accent" : "text-text1"}`}
                  >
                    {tool.label}
                  </div>
                  <div className="text-[0.65rem] text-text3 leading-tight">
                    {tool.desc}
                  </div>
                </div>
              </motion.button>
...
function TestStep({
  ...
  runLoading: boolean;
  runResult: ReturnType<
    typeof useSelector<RootState, RootState["agent"]["runResult"]>
  >;
  onTest: () => void;
  hasId: boolean;
}) {
      ...
      <div className="bg-accent-lt border border-accent/25 rounded-xl p-3.5 flex items-start gap-2.5">
        ...
        <div>
          <div className="text-[0.82rem] font-semibold text-accent">
            Testing: {draft.name || "Unnamed Agent"}
          </div>
          <div className="text-[0.72rem] text-text2 mt-0.5">
            {hasId
              ? "Send a message to see how your agent responds."
              : "Save your agent on the previous step first to enable testing."}
          </div>
        </div>
      </div>
      ...
      {runResult && (
        ...
        >
          <div className="bg-bg px-4 py-2.5 border-b border-black/[0.08] flex items-center gap-2">
            ...
            <span className="text-[0.75rem] font-semibold text-text1">
              {runResult.model.name}
            </span>
            <span className="ml-auto text-[0.65rem] text-text3">
          ...
          <div className="px-4 py-3">
            ...
            <div className="text-[0.8rem] text-text1 leading-relaxed whitespace-pre-wrap">
              {runResult.output}
            </div>
            {runResult.toolsUsed.length > 0 && (
              <div className="mt-2.5 flex flex-wrap gap-1">
                {runResult.toolsUsed.map((t) => (
                  <span
                    key={t}
                    className="text-[0.65rem] bg-bg2 text-text2 px-2 py-0.5 rounded-full"
                  >
                    {t}
                  </span>
                ))}
...
function DeployStep({ catalog }: { catalog: Model[] }) {
  ...
  const summary = [
    ...
    {
      label: "Tools",
      value: draft.tools.length > 0 ? draft.tools.join(", ") : "None",
    },
    {
      label: "System Prompt",
      value: draft.systemPrompt
        ? `${draft.systemPrompt.slice(0, 60)}…`
        : "None set",
    },
  ...
  return (
    <div className="space-y-4">
      <div className="bg-bg border border-black/[0.08] rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-black/[0.08]">
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{ background: model?.bg ?? "#F3F4F6" }}
            >
              {model?.icon ?? "🤖"}
            </div>
            <div>
              <div className="font-syne font-bold text-[0.95rem] text-text1">
        ...
        <div className="divide-y divide-black/[0.06]">
          {summary.map((s) => (
            ...
            >
              <span className="text-[0.75rem] text-text3 flex-shrink-0">
                {s.label}
              </span>
              <span className="text-[0.75rem] text-text1 font-medium text-right">
                {s.value}
              </span>
            </div>
          ))}
      ...
      <div className="bg-green/10 border border-green/20 rounded-xl p-3.5 flex items-start gap-2.5">
        ...
        <div>
          <div className="text-[0.82rem] font-semibold text-green">
            Ready to deploy
          </div>
          <div className="text-[0.72rem] text-text2 mt-0.5">
            Clicking deploy will set this agent to <strong>Active</strong> and
            make it available in your agents list.
          </div>
        </div>
      </div>
    </div>
