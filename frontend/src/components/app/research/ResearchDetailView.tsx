...
const CATEGORY_BADGE =
  "text-[0.65rem] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full bg-blue-lt text-blue";
export default function ResearchDetailView({
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-bg px-4 sm:px-6 md:px-8 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-text2 text-[0.9rem]">
  ...
  const arxivLink = article.arxivId
    ? `https://arxiv.org/abs/${article.arxivId}`
    : null;
  return (
        ...
        >
          <header className="mb-8">
            <p className="text-[0.8rem] text-text2 mb-2">
              ...
              <span
                className={`inline-flex align-middle ml-2 ${CATEGORY_BADGE}`}
              >
                {article.category}
              </span>
            </p>
            ...
            <p className="text-[0.8rem] sm:text-[0.85rem] text-text2">
              {article.authorsLine}
            </p>
          </header>

          <section className="mb-8">
            <h2 className="text-[0.7rem] font-semibold text-text3 uppercase tracking-widest mb-3">
              Overview
            </h2>
            <p className="text-[0.88rem] sm:text-[0.92rem] text-text2 leading-relaxed">
              {article.overview}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5">
              {article.metrics.map((m) => (
                ...
                >
                  <div className="font-syne text-xl sm:text-2xl font-bold text-text1">
                    {m.value}
                  </div>
                  <div className="text-[0.72rem] text-text3 mt-0.5">
                    {m.label}
                  </div>
                </div>
              ))}
            ...
            <ol className="space-y-3 list-none pl-0">
              {article.keyFindings.map((text, i) => (
                <li
                  ...
                  style={{ borderRadius: 12 }}
                >
                  <span className="font-syne font-bold text-accent flex-shrink-0 w-6">
                    {i + 1}
                  </span>
                  <span>{text}</span>
                </li>
          ...
          <section className="mb-8">
            ...
            >
              ...
              <p className="text-[0.88rem] text-text2 leading-relaxed">
                {article.impact}
              </p>
            </div>
          </section>
          ...
          <section className="mb-4">
            ...
            <div className="rounded-xl bg-bg2 border border-black/[0.08] px-4 py-3 flex flex-col sm:flex-row sm:items-start gap-3">
              <p className="text-[0.8rem] text-text2 leading-relaxed flex-1 font-mono">
                {article.citation}
              </p>
              <button
                type="button"
