...
function ResearchItemSkeleton() {
  return (
    <div
      className="bg-white border border-black/[0.08] p-4 sm:p-5 flex gap-3 sm:gap-4"
...
export default function ResearchView() {
  ...
  const [categoryFilter, setCategoryFilter] =
    useState<ResearchFilterKey>("all");
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 bg-bg">
      <div className="max-w-full sm:max-w-2xl md:max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-6">
          <div>
            <h2
              className="font-syne text-[1.3rem] sm:text-[1.6rem] font-bold text-text1"
              style={{ letterSpacing: "-0.03em" }}
            >
              AI Research Feed
            </h2>
            <p className="text-[0.8rem] sm:text-[0.85rem] text-text2 mt-1">
              Latest papers, releases & breakthroughs.
            </p>
          </div>
                ...
                >
                  <div className="flex gap-3 sm:gap-4">
                    <div className="flex-shrink-0 text-center w-10 sm:w-12">
                      <div className="text-[0.6rem] sm:text-[0.65rem] font-semibold uppercase text-text3">
                        {month}
                      </div>
                      <div className="font-syne text-lg sm:text-xl font-bold text-text1">
                        {day}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span
                          className={`text-[0.68rem] sm:text-[0.72rem] font-semibold px-2 py-0.5 rounded-full ${orgColor}`}
                        >
                          {item.org}
                        </span>
                      ...
                      <h3
                        className="font-syne font-bold text-text1 text-[0.88rem] sm:text-[0.95rem] mb-1.5 group-hover:text-accent transition-colors leading-snug"
                        style={{ letterSpacing: "-0.01em" }}
                      >
                        {item.title}
                      </h3>
                      <p className="text-[0.78rem] sm:text-[0.82rem] text-text2 leading-relaxed hidden sm:block">
                        {item.summary}
                      </p>
                      <p className="text-[0.75rem] text-text2 leading-relaxed sm:hidden line-clamp-2">
                        {item.summary}
                      </p>
                      <button
                        type="button"
        ...
        >
          ...
          <h3
            className="font-syne text-lg sm:text-xl font-bold mb-2"
            style={{ letterSpacing: "-0.02em" }}
          >
            Stay ahead of AI
          </h3>
          <p className="text-white/60 text-[0.8rem] sm:text-[0.85rem] mb-4 sm:mb-5">
            Weekly digest of the most important AI research.
          </p>
