...
export default function ChatModelsSidebar() {
  ...
  const handleSelect = (id: string) => {
    ...
    if (model)
      dispatch(
        showToast(t("chat.sidebar.switched_to_toast", { name: model.name })),
      );
  };
  return (
      ...
      <div className="p-2 sm:p-3 flex-1">
        ...
        {filtered.map((model) => (
          <button
            ...
            className={`w-full flex items-center gap-2 px-2.5 sm:px-3 py-2 cursor-pointer transition-colors mb-0.5 text-left ${currentModelId === model.id ? "bg-accent-lt" : "hover:bg-bg2"}`}
            style={{ borderRadius: 8 }}
          >
            <div
            ...
            <div className="overflow-hidden min-w-0">
              <div className="text-[0.78rem] sm:text-[0.82rem] font-medium text-text1 truncate">
                {model.name}
              </div>
              <div className="text-[0.62rem] sm:text-[0.68rem] text-text3">
                <span
                  className={`w-1.5 h-1.5 rounded-full inline-block mr-1 ${model.badge === "new" ? "bg-green" : "bg-amber"}`}
                />
                {model.org}
              </div>
            </div>
