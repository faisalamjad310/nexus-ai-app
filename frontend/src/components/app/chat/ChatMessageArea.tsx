...
export default function ChatMessageArea() {
  ...
  const { messages, onboardPhase, obDone, isTyping, userGoal } = useSelector(
    (s: RootState) => s.chat,
  );
  ...
  const getAiReplyLocal = (
    ...
    if (
      textLow.includes("code") ||
      textLow.includes("coding") ||
      textLow.includes("programming")
    )
      return {
        text: t("chat.sidebar.loading"),
        recs: catalog.filter((m) => m.types.includes("code")).slice(0, 3),
      };
  ...
  const handleOnboardPick = async (phase: string, value: string) => {
    ...
    if (phase === "audience") {
      setTimeout(() => {
        dispatch(
          addMessage({
            id: (Date.now() + 1).toString(),
            role: "ai",
            content: t("chat.area.onboarding.level.title"),
            timestamp: Date.now() + 1,
          }),
        );
      ...
      setTimeout(() => {
        dispatch(
          addMessage({
            id: (Date.now() + 1).toString(),
            role: "ai",
            content: t("chat.area.onboarding.budget.title"),
            timestamp: Date.now() + 1,
          }),
        );
      ...
      try {
        const { text: aiText, recs: rawRecs } = await apiChatMessage(
          userGoal || "AI assistant",
          {
            goal: userGoal || undefined,
            budget: value,
          },
        );
        const recs = (rawRecs as Model[]).map(
          (r) => catalog.find((m) => m.id === r.id) ?? r,
        );
        ...
        const recs = catalog
          .filter((m) => {
            if (value === t("chat.area.onboarding.budget.free"))
              return m.price_start === 0;
            if (value === t("chat.area.onboarding.budget.under50"))
              return m.price_start < 5;
            return true;
          })
          .slice(0, 3);
    ...
    <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-4 sm:py-6 flex flex-col gap-3 sm:gap-4 items-start bg-bg">
      {showGreeting && (
        ...
        >
          ...
          <h3
            className="font-syne text-[1.1rem] sm:text-[1.3rem] font-bold text-text1 text-center mb-2"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {GOAL_TILES.map((tile) => (
                ...
                >
                  ...
                  <span className="text-[0.72rem] sm:text-xs font-medium text-text2 text-center leading-tight">
                    {tile.label}
                  </span>
                </motion.button>
              ))}
      ...
      <AnimatePresence>
        {messages.map((msg) => (
          ...
          >
            <div
              className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-semibold flex-shrink-0 mt-0.5 ${msg.role === "user" ? "bg-accent text-white" : "bg-accent-lt text-accent border border-accent/25 text-base"}`}
            >
              {msg.role === "user" ? "U" : "✦"}
            </div>
            <div className="min-w-0 flex-1">
              ...
              {msg.attachments && msg.attachments.length > 0 && (
                <div
                  className={`mt-1.5 flex flex-col gap-2 ${msg.role === "user" ? "items-end" : "items-start"}`}
                >
                  <div
                    className={`flex flex-wrap gap-1.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}
                  >
                    {msg.attachments.map((file) => {
                        ...
                        return (
                          ...
                          >
                            <Image
                              ...
                              height={320}
                              unoptimized
                              className="max-h-40 h-auto w-auto object-contain"
                            />
                          </a>
                        );
                      }
                      if (isAudio) {
                        return (
                          <div
                            key={file.id}
                            className="max-w-full rounded-lg border border-black/[0.12] bg-bg p-2"
                          >
                            <div className="text-[0.65rem] text-text3 mb-1 truncate max-w-[240px]">
                              {file.name}
                            </div>
                            <audio
                              controls
                      ...
                      return (
                        ...
                        >
                          {file.url ? (
                            <a
                              href={resolveApiPublicUrl(file.url)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="underline"
                            >
                  ...
                  {msg.recs.map((rec) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="text-[0.8rem] font-medium text-text1 truncate">
                          {rec.name}
                        </div>
                        <div className="text-[0.68rem] text-text3">
                          {rec.org}
                        </div>
                      </div>
                    </motion.div>
      ...
      {(onboardPhase === "audience" ||
        ...
        >
          {ONBOARD_QUESTIONS.find((q) => q.phase === onboardPhase)?.opts.map(
            (opt) => (
              <motion.button
                key={opt.label}
                whileHover={{
      ...
      {isTyping && (
        ...
        >
          <div className="w-8 h-8 rounded-full bg-accent-lt text-accent border border-accent/25 flex items-center justify-center text-base">
            ✦
          </div>
          <div
            className="bg-white border border-black/[0.08] rounded-lg px-4 py-3 flex items-center gap-1.5"
            style={{ borderRadius: 12 }}
          >
            {[0, 0.2, 0.4].map((d, i) => (
              <motion.span
