...
function getAiReply(txt: string): { text: string; recs?: typeof MODELS } {
  const t = txt.toLowerCase();
  if (t.includes('code') || t.includes('coding') || t.includes('programming'))
    return { text: 'For coding tasks, here are the top models:', recs: MODELS.filter((m) => m.types.includes('code')).slice(0, 3) };
  if (t.includes('image') || t.includes('vision') || t.includes('picture'))
    return { text: 'For image and vision tasks:', recs: MODELS.filter((m) => m.types.includes('image') || m.types.includes('vision')).slice(0, 3) };
  if (t.includes('cheap') || t.includes('free') || t.includes('budget'))
    return { text: 'Best value models:', recs: MODELS.filter((m) => m.price_start === 0 || m.price_start < 1).slice(0, 3) };
  if (t.includes('fast') || t.includes('speed'))
    return { text: 'Fastest response models:', recs: MODELS.filter((m) => m.tags.some((tag) => tag.toLowerCase().includes('fast'))).slice(0, 3) };
  if (t.includes('open') || t.includes('self-host'))
    return { text: 'Top open-source self-hostable models:', recs: MODELS.filter((m) => m.types.includes('open')).slice(0, 3) };
  if (t.includes('agent'))
    return { text: 'Best models for building AI agents:', recs: MODELS.filter((m) => m.tags.some((t) => t.toLowerCase().includes('agent'))).slice(0, 3) };
  return { text: `Based on your query about "${txt}", here are my top picks:`, recs: MODELS.slice(0, 3) };
}
  ...
  const handleOnboardPick = async (phase: string, value: string) => {
    ...
    if (phase === 'audience') {
      ...
      try {
        ...
        const recs = (rawRecs as typeof MODELS).map((r) => MODELS.find((m) => m.id === r.id) ?? r);
        dispatch(setPendingRecs(recs as typeof MODELS));
        dispatch(setIsTyping(false));
        dispatch(addMessage({
          id: (Date.now() + 1).toString(), role: 'ai',
          content: aiText,
          recs: recs as typeof MODELS,
          timestamp: Date.now() + 1,
        }));
      } catch {
        const recs = MODELS.filter((m) => {
