...
const getQuickActions = (t: any) => [
  {
    label: t("chat.right_panel.actions.marketplace"),
    tab: "marketplace" as const,
  },
  { label: t("chat.right_panel.actions.build_agent"), tab: "agents" as const },
  {
    label: t("chat.right_panel.actions.research_feed"),
    ...
    <div className="w-[272px] flex-shrink-0 bg-white border-l border-black/[0.08] overflow-y-auto flex flex-col">
      {currentModel && (
        <div className="p-4 border-b border-black/[0.08]">
          ...
          <div
            className="bg-bg border border-black/[0.14] rounded-sm p-3.5"
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                <h4 className="text-[0.875rem] font-semibold font-syne text-text1">
                  {currentModel.name}
                </h4>
                <small className="text-[0.7rem] text-text3">
                  {currentModel.org}
                </small>
              </div>
            </div>
            <p className="text-[0.75rem] text-text2 leading-relaxed mb-3">
              {currentModel.desc.slice(0, 100)}...
            </p>
            <div className="grid grid-cols-3 gap-1.5 mb-3">
              {[
                {
                  label: t("chat.right_panel.rating"),
                  value: `${currentModel.rating}⭐`,
                },
                {
                  label: t("chat.right_panel.reviews"),
                  value: `${(currentModel.reviews / 1000).toFixed(1)}k`,
                },
                {
                  label: t("chat.right_panel.price"),
                  value: currentModel.price.split("/")[0],
                },
              ].map((s) => (
                <div
                  key={s.label}
                  className="bg-white border border-black/[0.08] rounded text-center p-1.5"
                >
                  <strong className="block text-[0.85rem] font-semibold text-text1">
                    {s.value}
                  </strong>
            ...
            <div className="flex gap-1.5">
              ...
              <button
                onClick={() =>
                  dispatch(openModal({ model: currentModel, tab: "guide" }))
                }
      ...
      <div className="p-4">
        ...
        {[
          {
            org: "OpenAI",
            text: "GPT-5.4 with native computer use",
            time: "2h ago",
          },
          {
            org: "Anthropic",
            text: "Claude 4.6 Opus with 1M context",
            time: "1d ago",
          },
          {
            org: "Google",
            text: "Gemini 3.1 Pro with 5M context",
            time: "3d ago",
          },
        ].map((u, i) => (
          <div
            key={i}
            className="border border-black/[0.08] rounded-sm p-2.5 mb-1.5 cursor-pointer hover:border-black/[0.14] hover:shadow-card transition-all"
            style={{ borderRadius: 8 }}
          >
            <div className="flex items-center gap-1.5 mb-1">
              <span className="text-[0.68rem] font-semibold text-accent bg-accent-lt px-1.5 py-0.5 rounded">
                {u.org}
              </span>
              <span className="text-[0.65rem] text-text3">{u.time}</span>
            </div>
            <div className="text-[0.75rem] text-text2 leading-snug">
              {u.text}
            </div>
          </div>
        ))}
